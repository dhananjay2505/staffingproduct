...
/*	delete section in model*/
	...
	public function updateeducationdatamodal($u_token, $user_token, $classname, $schoolname, $startingyear){
		$timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));
		$data = array(
	...
	public function inserteducationmodel($token, $user_token, $classname, $schoolname, $startingyear){
		$timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));
	...
	/*ducation section function*/

	public function educationdatagetmodel($token){
		$query = $this->db->where('user_token', $token)
						->get('academic_certifications');
		if ($query->num_rows()>0) {
	...
	/*insert one time education section*/

	public function firstinsertecidationsection($token, $user_token, $qilification, $universityname, $coursename, $passingyear, $passingendyear){
		$timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));

		$data = array(
			'academic_certifications_token' => "$token",
		    'user_token' => "$user_token",
		    'academic_certifications_name' => "$qilification",
		    'academic_certifications_school_name' => "$universityname",
		    'academic_certifications_course_type' => "$coursename",
		    'academic_certifications_passing_year' => "$passingyear",
		    'academic_certifications_created_at' => "$current_time",
		    'passing_month' => "$passingendyear",
		);
		return $this->db->insert('academic_certifications', $data);
		
	}

	public function addmorefirstinsertecidationsection($token, $user_token, $qilification, $universityname, $coursename, $passingyear, $passingendyear){
		$timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));
	...
	/*fetcheducation section data*/

	public function educationsetionfetchmodal($edu_token, $user_token){
		$query = $this->db->where('academic_certifications_token', $edu_token)
						->where('user_token', $user_token)
						->get('academic_certifications');
		if ($query->num_rows()>0) {
	...
	/*delete eduaction section*/
	public function educationdeleteexprencedatamodal($u_token, $user_token){
		if ($this->db->where('academic_certifications_token', $u_token)
				->where('user_token', $user_token)
				->delete('academic_certifications')) {
			return true;
		}else{
	...
	public function updateeducationsection($token, $user_token, $qilification, $universityname, $coursename, $passingyear, $passingendyear){
		$timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));
		...
		$data = array(
		    ...
		    'academic_certifications_course_type' => "$coursename",
		    'academic_certifications_passing_year' => "$passingyear",
		    'academic_certifications_updated_at' => "$current_time",
		    'passing_month' => "$passingendyear",
		);
		if ($this->db->where('academic_certifications_token', $token)
						->where('user_token', $user_token)
					->update('academic_certifications', $data)) {
			return true;
		}else{
			return false;
		}
	...
	/*proof indentification*/

	public function save_upload_model($user_token,$image,$token){
        $timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));
		$da = $this->db->where('user_token', $user_token)
		...
		if ($da->num_rows() > 0) {
		    ...
		    'candidate_profile_created_at' => "$current_time"
		 	...
		 	$this->db->insert('candidate_profile', $data);
		 	return false;
		}
   	...
   	public function deleteimageadhar($user_token){
   		$data = array(
   			'candidate_profile_adharcard' => NULL,
		);
		if ($this->db->where('user_token', $user_token)
					->update('candidate_profile', $data)) {
			return true;
   	...
   	public function getimagedata($user_token){
   		$da = $this->db->where('user_token', $user_token)
				->get('candidate_profile');
		if ($da->num_rows() > 0) {
			return $da->result();
   ...
   public function profilegetimageadhar($user_token){
   	$da = $this->db->where('user_token', $user_token)
				->get('candidate_profile');
		if ($da->num_rows() > 0) {
			return $da->result();
   ...
   public function profile_save_upload_model($user_token,$image,$token){
        $timenow=time();
		date_default_timezone_set("Asia/Kolkata");
		$current_time = (date("Y-m-d h:i:s",$timenow));
		...
		if ($da->num_rows() > 0) {
			$data = array(
                'candidate_profile_image' => $image,
                'candidate_profile_upload_time' => $current_time
            );
            $this->db->where('user_token',$user_token)
        					->update('candidate_profile', $data);
        	return true;	
    ...
    public function peofiledeleteimageadhar($user_token){
   		$data = array(
   ...
   public function getstate(){
   		$data = $this->db->select('city_state')
   ...
   public function fetchcitymodal($state){
   	$data = $this->db->where('city_state', $state)
